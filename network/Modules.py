...
class GC3d(nn.Module):
  def __init__(self, inplanes, planes, kh=7, kw=7):
    ...
    self.conv_l1 = nn.Conv3d(inplanes, 256, kernel_size=(kh, 1,1),
                             padding=(int(kh / 2), 0, 0))
    self.conv_l2 = nn.Conv3d(256, planes, kernel_size=(1, kw,1),
                             padding=(0, int(kw / 2), 0))
    self.conv_r1 = nn.Conv3d(inplanes, 256, kernel_size=(1, kw, 1),
                             padding=(0, int(kw / 2), 0))
    self.conv_r2 = nn.Conv3d(256, planes, kernel_size=(kh, 1, 1),
                             padding=(int(kh / 2), 0, 0))
