...
def read_flow(filename):
    ...
    if 202021.25 != magic:
        ...
        data2d = np.fromfile(f, np.float32, count=(2 * w * h)[0])
...
def warp_image_torch(im, flow):
    ...
    flow_height = flow.shape[-2]
    flow_width = flow.shape[-1]
    n = image_height * image_width
    (iy, ix) = torch.meshgrid([torch.range(0,image_height - 1),torch.range(0,image_width - 1)])
    (fy, fx) = torch.meshgrid([torch.range(0, flow_height - 1),torch.range(0, flow_width - 1)])

    fx = fx.cuda()
    fy = fy.cuda()
    fx += flow[0, 0, :, :]
    fy += flow[0, 1,:, :]
    mask = (((fx < 0) + (fx>flow_width)) != 0)
    mask = (mask + (fy < 0)) != 0
    mask = (mask + (fy > flow_height)) !=0
    fx = torch.min(torch.max(fx, torch.zeros_like(fx)), torch.ones_like(fx) * flow_width)
    fy = torch.min(torch.max(fy, torch.zeros_like(fy)), torch.ones_like(fy) * flow_height)
    points = torch.cat((ix.reshape(n, 1), iy.reshape(n, 1)), dim=1)
    xi = torch.cat((fx.reshape(n, 1), fy.reshape(n, 1)), dim=1)
    warp = torch.zeros((image_height, image_width, im.shape[1]))
    for i in range(im.shape[1]):
        channel = im[0, i, :, :]
        # plt.imshow(channel, cmap='gray')
        values = channel.reshape(n, 1)
        new_channel = interpolate.griddata(points.data.cpu(), values.data.cpu(), xi.data.cpu(), method='cubic')
        new_channel = torch.from_numpy(new_channel).reshape([flow_height, flow_width])
        new_channel[mask] = 1
        warp[:, :, i] = new_channel.int()
